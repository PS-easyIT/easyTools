...
# Implementierung der Update-ScriptList Funktion mit verbesserter Pfadbehandlung
# Verbesserte Update-ScriptList Funktion für zuverlässige Anzeige der PS1-Dateien
function Update-ScriptList {
    try {
        ...
        # Definiere mehrere Methoden, um den Basispfad zu ermitteln (in Prioritätsreihenfolge)
        $baseFolder = $null
        
        # 1. Explizite Pfadangabe (höchste Priorität)
        $explicitPath = "Z:\OD_PowerShell"
        if (Test-Path -Path $explicitPath -ErrorAction SilentlyContinue) {
            $baseFolder = $explicitPath
            Update-LogDisplay -Message "Verwende definierten Basispfad: $baseFolder" -Type "Info"
        }
        # 2. Script-Verzeichnis über $PSScriptRoot
        elseif ($PSScriptRoot) {
            $baseFolder = $PSScriptRoot
            Update-LogDisplay -Message "Verwende PSScriptRoot: $baseFolder" -Type "Info"
        }
        # 3. Aktuelles Verzeichnis als letzter Ausweg
        else {
            $baseFolder = (Get-Location).Path
            Update-LogDisplay -Message "Verwende aktuelles Verzeichnis: $baseFolder" -Type "Warning"
        }
        
        # Pfade definieren, in denen nach Skripten gesucht werden soll
        $scriptFolders = @($baseFolder)
        
        # Unterordner prüfen und zum Durchsuchen hinzufügen
        ...
        foreach ($subFolder in $potentialSubFolders) {
            ...
            if (Test-Path -Path $fullPath -ErrorAction SilentlyContinue) {
                ...
                Update-LogDisplay -Message "Gefundener Unterordner: $fullPath" -Type "Info"
            } else {
                # Versuchen, den Unterordner zu erstellen
                try {
                    New-Item -Path $fullPath -ItemType Directory -Force -ErrorAction Stop | Out-Null
                    $scriptFolders += $fullPath
                    Update-LogDisplay -Message "Unterordner erstellt: $fullPath" -Type "Success"
                } catch {
                    Update-LogDisplay -Message "Konnte Unterordner nicht erstellen: $fullPath - $($_.Exception.Message)" -Type "Warning"
                }
            }
        }
        ...
        # Skripte sammeln mit verbesserter Fehlerbehandlung
        $scripts = @()
        $totalScripts = 0
        
        foreach ($folder in $scriptFolders) {
            try {
                # Prüfe, ob Verzeichnis existiert und zugänglich ist
                if (Test-Path -Path $folder -PathType Container -ErrorAction Stop) {
                    # Verwende eine robustere Methode zum Abrufen der Dateien
                    $folderScripts = Get-ChildItem -Path $folder -Filter "*.ps1" -File -ErrorAction Stop
                    
                    # Aktuelles Skript ausfiltern
                    $currentScriptName = if ($MyInvocation.MyCommand.Name) { $MyInvocation.MyCommand.Name } else { "easyStartUp_V0.0.2.ps1" }
                    $filteredScripts = $folderScripts | Where-Object { $_.Name -ne $currentScriptName }
                    
                    if ($filteredScripts -and $filteredScripts.Count -gt 0) {
                        $scripts += $filteredScripts
                        $totalScripts += $filteredScripts.Count
                        Update-LogDisplay -Message "Gefundene Skripte in $folder - $($filteredScripts.Count)" -Type "Success"
                    } else {
                        Update-LogDisplay -Message "Keine relevanten Skripte in $folder gefunden" -Type "Info"
                    }
                } else {
                    Update-LogDisplay -Message "Ordner nicht zugänglich - $folder" -Type "Warning"
                }
            } catch {
        ...
        # Wenn keine Skripte gefunden wurden, Beispielskript erstellen
        if ($scripts.Count -eq 0) {
            Update-LogDisplay -Message "Keine PowerShell-Skripte gefunden. Erstelle Beispielskript..." -Type "Warning"
            
            try {
                $exampleScriptPath = Join-Path -Path $baseFolder -ChildPath "BeispielSkript.ps1"
...
.DESCRIPTION
    Dies ist ein Beispielskript, das automatisch erstellt wurde, 
    weil keine anderen PowerShell Skripte gefunden wurden.
#>

Write-Host "Hallo von BeispielSkript.ps1!" -ForegroundColor Green
Write-Host "Sie können dieses Skript als Vorlage verwenden oder eigene Skripts erstellen."
"@
                
                Set-Content -Path $exampleScriptPath -Value $exampleContent -ErrorAction Stop
                Update-LogDisplay -Message "Beispielskript erstellt: $exampleScriptPath" -Type "Success"
                
                # Neues Skript zur Liste hinzufügen
                $scripts += Get-Item -Path $exampleScriptPath -ErrorAction Stop
                $totalScripts++
            } catch {
                Update-LogDisplay -Message "Fehler beim Erstellen des Beispielskripts: $($_.Exception.Message)" -Type "Error"
            }
            
            # Wenn immer noch keine Skripte, Platzhalter anzeigen
            if ($scripts.Count -eq 0) {
                $textBlock = New-Object System.Windows.Controls.TextBlock
                $textBlock.Text = "Keine Skripte gefunden. Bitte erstellen Sie Skripte im Verzeichnis $baseFolder"
        ...
        foreach ($script in $scripts) {
            ...
            if ($parentDir -ne (Split-Path -Leaf $baseFolder)) {
                $category = $parentDir
            }
        ...
        foreach ($category in ($scriptsByCategory.Keys | Sort-Object)) {
            ...
            foreach ($script in $scriptsByCategory[$category]) {
                ...
                try {
                    ...
                    # Fehler bei Beschreibungsextraktion ignorieren
                }
                ...
                # Click-Event für das Ausführen des Skripts
                $button.Add_Click({
        ...
        Update-LogDisplay -Message "Skriptliste wurde geladen - $totalScripts Skripte gefunden." -Type "Success"
    }
    catch {
...
# Alias für Update-ScriptList verbessert
...
try {
    ...
    # Skriptliste sofort beim Start laden (wichtig!)
    Update-ScriptList
...
function Update-ScriptList {
    try {
        ...
        # Basisordner definieren - da $PSScriptRoot Probleme macht, verwenden wir einen absoluten Pfad
        $baseFolder = "Z:\OD_PowerShell"
        
        # Sicherstellen, dass der Pfad existiert
        if (-not (Test-Path -Path $baseFolder -ErrorAction SilentlyContinue)) {
            # Versuchen, das Verzeichnis zu erstellen
            try {
                New-Item -Path $baseFolder -ItemType Directory -Force -ErrorAction Stop | Out-Null
                Update-LogDisplay -Message "Basisordner wurde erstellt: $baseFolder" -Type "Info"
            } catch {
                Update-LogDisplay -Message "Basis-Verzeichnis konnte nicht erstellt werden: $($_.Exception.Message)" -Type "Error"
                
                # Fallback auf aktuelles Verzeichnis
                $baseFolder = (Get-Location).Path
                Update-LogDisplay -Message "Verwende aktuelles Verzeichnis als Fallback: $baseFolder" -Type "Warning"
            }
        }
        ...
        foreach ($subFolder in $potentialSubFolders) {
            ...
            if (Test-Path -Path $fullPath -ErrorAction SilentlyContinue) {
                ...
                # Versuche Unterordner zu erstellen
                try {
                    New-Item -Path $fullPath -ItemType Directory -Force -ErrorAction Stop | Out-Null
                    $scriptFolders += $fullPath
                    Update-LogDisplay -Message "Unterordner erstellt und zur Suche hinzugefügt: $fullPath" -Type "Info"
                } catch {
                    Update-LogDisplay -Message "Unterordner konnte nicht erstellt werden: $fullPath" -Type "Warning"
                }
            }
        }
        ...
        foreach ($folder in $scriptFolders) {
            if (-not [string]::IsNullOrEmpty($folder) -and (Test-Path -Path $folder -ErrorAction SilentlyContinue)) {
                ...
                Update-LogDisplay -Message "Ordner existiert nicht oder ist nicht zugänglich: $folder" -Type "Warning"
            }
        }
...
function Connect-ToPnPOnline {
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($Url)) {
            Update-LogDisplay -Message "Zeige PnP SharePoint Site URL Dialog an..." -Type "Info"
            
            # Statt Windows.Forms verwenden wir WPF für Konsistenz
            $spoUrl = Show-SPOConnectDialog
...
function Connect-ToSharePointOnline {
    ...
    try {
        ...
        try {
            # Alternatives Importieren, um Kompatibilitätsproblem zu umgehen
            $module = Get-Module -Name Microsoft.Online.SharePoint.PowerShell -ListAvailable | 
                      Sort-Object Version -Descending | Select-Object -First 1
            
            if ($module) {
                # Prüfe auf alte Version, die besser funktionieren könnte
                $olderModule = Get-Module -Name Microsoft.Online.SharePoint.PowerShell -ListAvailable | 
                               Where-Object { $_.Version -lt $module.Version } | 
                               Sort-Object Version -Descending | Select-Object -First 1
                
                if ($olderModule -and $module.Version.Major -gt $olderModule.Version.Major) {
                    Update-LogDisplay -Message "Versuche ältere Version des SharePoint-Moduls zu laden: $($olderModule.Version)" -Type "Info"
                    Import-Module -ModuleInfo $olderModule -DisableNameChecking -Force -ErrorAction Stop
                } else {
                    Import-Module -ModuleInfo $module -DisableNameChecking -Force -ErrorAction Stop
                }
                
                Update-LogDisplay -Message "SharePoint Online PowerShell Modul importiert: $($module.Version)" -Type "Info"
            } else {
                throw "Kein SharePoint Online PowerShell Modul gefunden"
            }
        }
        catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC