...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="easyADReport v0.2.4" Height="950" Width="1650"
        ...
        <Border Grid.Row="0" Background="#0078D7" BorderThickness="0">
            <Grid>
                ...
                <StackPanel Grid.Column="0" Orientation="Horizontal" Margin="20,0">
                    ...
                    <TextBlock Text="v0.2.4" Foreground="#E1E1E1" FontSize="14" VerticalAlignment="Center" Margin="10,0,0,0"/>
                </StackPanel>
                        ...
                        <StackPanel>
                            ...
                            <GroupBox Header="AD-Health" FontSize="11" BorderThickness="0" Margin="0,5,0,0">
                                <StackPanel>
                                    ...
                                    <Button x:Name="ButtonQuickDNSZones" Content="DNS Zones" Margin="0,2" FontSize="10" Height="15" Background="#F5F5F5" BorderBrush="#DDDDDD" HorizontalAlignment="Stretch" HorizontalContentAlignment="Left"/>
                                </StackPanel>
                            </GroupBox>
                        </StackPanel>
                    </GroupBox>
        ...
        <Border Grid.Row="2" Background="#F0F0F0" BorderThickness="0,1,0,0" BorderBrush="#DDDDDD">
            <Grid Margin="20,0">
                ...
                <Button x:Name="ButtonClose" Content="EXIT" 
                            Background="#FFFF4242" Foreground="White" BorderThickness="0" FontWeight="Bold" FontSize="16" 
                            HorizontalAlignment="Right" Width="100" Margin="0,5,10,8">
                    <Button.Resources>
                        <Style TargetType="{x:Type Border}">
                            <Setter Property="CornerRadius" Value="4"/>
                        </Style>
...
# --- Funktion zum Abrufen von Benutzern die sich nie angemeldet haben (Quick Report) ---
Function Get-ADReportNeverLoggedOnUsers {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Benutzer die sich nie angemeldet haben' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "PasswordLastSet", "DistinguishedName", "Created", 
            "Modified", "Description", "mail", "ObjectClass", "LogonCount"
        )
        
        # Benutzer die sich nie angemeldet haben (LogonCount = 0 oder LastLogonDate = null)
        $NeverLoggedOnUsers = Get-ADUser -Filter 'Enabled -eq $true' -Properties $Properties -ErrorAction SilentlyContinue | 
            Where-Object { ($_.LogonCount -eq 0 -or $_.LogonCount -eq $null) -and ($_.LastLogonDate -eq $null) }
        
        if ($NeverLoggedOnUsers) {
            Write-ADReportLog -Message "$($NeverLoggedOnUsers.Count) Benutzer gefunden, die sich nie angemeldet haben." -Type Info -GUI
            return $NeverLoggedOnUsers | Select-Object $Properties
        } else {
            Write-ADReportLog -Message "Keine Benutzer gefunden, die sich nie angemeldet haben." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Nie angemeldete Benutzer' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von kürzlich gelöschten Benutzern (Quick Report) ---
Function Get-ADReportRecentlyDeletedUsers {
    [CmdletBinding()]
    param(
        [int]$Days = 30 # Standardmäßig die letzten 30 Tage
    )

    Write-ADReportLog -Message "Lade 'Kürzlich gelöschte Benutzer (letzte $($Days) Tage)' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", 
            "DistinguishedName", "Created", "Modified", "Description", 
            "mail", "ObjectClass", "whenChanged", "isDeleted", "msDS-LastKnownRDN"
        )
        $targetDate = (Get-Date).AddDays(-$Days)
        
        # Suche nach gelöschten Objekten im Deleted Objects Container
        try {
            $Domain = Get-ADDomain
            $DeletedObjectsContainer = "CN=Deleted Objects," + $Domain.DistinguishedName
            
            $RecentlyDeletedUsers = Get-ADObject -Filter {(objectClass -eq "user") -and (isDeleted -eq $true) -and (whenChanged -ge $targetDate)} `
                -Properties $Properties -IncludeDeletedObjects -SearchBase $DeletedObjectsContainer -ErrorAction SilentlyContinue
            
            if ($RecentlyDeletedUsers) {
                $ProcessedUsers = $RecentlyDeletedUsers | Select-Object @{
                    Name='DisplayName'; Expression={if ($_.DisplayName) {$_.DisplayName} else {"N/A"}}
                }, @{
                    Name='SamAccountName'; Expression={if ($_.'msDS-LastKnownRDN') {$_.'msDS-LastKnownRDN' -replace '^CN=',''} else {"N/A"}}
                }, @{
                    Name='GivenName'; Expression={if ($_.GivenName) {$_.GivenName} else {"N/A"}}
                }, @{
                    Name='Surname'; Expression={if ($_.Surname) {$_.Surname} else {"N/A"}}
                }, @{
                    Name='mail'; Expression={if ($_.mail) {$_.mail} else {"N/A"}}
                }, @{
                    Name='WhenDeleted'; Expression={$_.whenChanged}
                }, @{
                    Name='OriginalContainer'; Expression={$_.DistinguishedName}
                }, @{
                    Name='Description'; Expression={if ($_.Description) {$_.Description} else {"N/A"}}
                }
                
                Write-ADReportLog -Message "$($ProcessedUsers.Count) kürzlich gelöschte Benutzer (seit $($Days) Tagen) gefunden." -Type Info -GUI
                return $ProcessedUsers
            } else {
                Write-ADReportLog -Message "Keine kürzlich gelöschten Benutzer (seit $($Days) Tagen) gefunden." -Type Warning -GUI
                return @()
            }
        } catch {
            Write-ADReportLog -Message "Fehler beim Zugriff auf gelöschte Objekte: $($_.Exception.Message). Möglicherweise fehlen die erforderlichen Berechtigungen." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Kürzlich gelöschte Benutzer' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von inaktiven Benutzern mit anpassbarer Tageanzahl (Quick Report) ---
Function Get-ADReportInactiveUsersXDays {
    [CmdletBinding()]
    param(
        [int]$InactiveDays = 30 # Anpassbar über Dialog
    )

    # Dialog zur Eingabe der Tageanzahl
    Add-Type -AssemblyName Microsoft.VisualBasic
    $UserInput = [Microsoft.VisualBasic.Interaction]::InputBox("Geben Sie die Anzahl der Tage für inaktive Benutzer ein:", "Inaktive Benutzer", $InactiveDays.ToString())
    
    if ([string]::IsNullOrWhiteSpace($UserInput) -or -not [int]::TryParse($UserInput, [ref]$InactiveDays)) {
        Write-ADReportLog -Message "Ungültige Eingabe oder Vorgang abgebrochen." -Type Warning -GUI
        return @()
    }

    Write-ADReportLog -Message "Lade 'Inaktive Benutzer (letzte $($InactiveDays) Tage)' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "PasswordLastSet", "DistinguishedName", "Created", 
        ...
        if ($InactiveUsers) {
            ...
            $ProcessedUsers = $InactiveUsers | Select-Object *, @{
            ...
            Write-ADReportLog -Message "$($ProcessedUsers.Count) inaktive Benutzer (seit $($InactiveDays)+ Tagen) gefunden." -Type Info -GUI
            return $ProcessedUsers
        } else {
            Write-ADReportLog -Message "Keine inaktiven Benutzer (seit $($InactiveDays)+ Tagen) gefunden." -Type Warning -GUI
            return @()
        ...
        $ErrorMessage = "Fehler beim Laden des 'Inaktive Benutzer (X Tage)' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern ohne Manager (Quick Report) ---
Function Get-ADReportUsersWithoutManager {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Benutzer ohne Manager' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "PasswordLastSet", "DistinguishedName", "Created", 
            "Modified", "Description", "mail", "ObjectClass", "Manager", "Department", "Title"
        )
        
        # Benutzer ohne Manager-Attribut
        $UsersWithoutManager = Get-ADUser -Filter 'Enabled -eq $true -and Manager -notlike "*"' -Properties $Properties -ErrorAction SilentlyContinue
        
        if ($UsersWithoutManager) {
            Write-ADReportLog -Message "$($UsersWithoutManager.Count) Benutzer ohne Manager gefunden." -Type Info -GUI
            return $UsersWithoutManager | Select-Object $Properties
        } else {
            Write-ADReportLog -Message "Keine Benutzer ohne Manager gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Benutzer ohne Manager' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit fehlenden erforderlichen Attributen (Quick Report) ---
Function Get-ADReportUsersMissingRequiredAttributes {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Benutzer mit fehlenden erforderlichen Attributen' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "DistinguishedName", "Created", "Modified", 
            "Description", "mail", "ObjectClass", "Department", "Title", "Manager", 
            "OfficePhone", "UserPrincipalName"
        )
        
        # Alle aktiven Benutzer laden
        $AllUsers = Get-ADUser -Filter 'Enabled -eq $true' -Properties $Properties -ErrorAction SilentlyContinue
        
        # Erforderliche Attribute definieren (anpassbar je nach Unternehmensrichtlinien)
        $RequiredAttributes = @("DisplayName", "GivenName", "Surname", "mail", "UserPrincipalName")
        $OptionalImportantAttributes = @("Department", "Title", "Manager")
        ...
        foreach ($User in $AllUsers) {
            ...
            foreach ($Attr in $RequiredAttributes) {
                if ([string]::IsNullOrWhiteSpace($User.$Attr)) {
                    $MissingRequired += $Attr
                }
            }
            
            # Prüfe wichtige optionale Attribute
            foreach ($Attr in $OptionalImportantAttributes) {
                if ([string]::IsNullOrWhiteSpace($User.$Attr)) {
                    $MissingOptional += $Attr
                }
            }
            
            # Wenn fehlende Attribute vorhanden sind, Benutzer zur Liste hinzufügen
            if ($MissingRequired.Count -gt 0 -or $MissingOptional.Count -gt 0) {
                $UserWithMissing = $User | Select-Object *, @{
                    Name='MissingRequiredAttributes'; Expression={$MissingRequired -join ', '}
                }, @{
                    Name='MissingOptionalAttributes'; Expression={$MissingOptional -join ', '}
                }, @{
                    Name='Severity'; Expression={
                        if ($MissingRequired.Count -gt 0) { "Critical" }
                        elseif ($MissingOptional.Count -gt 2) { "High" }
                        else { "Medium" }
                    }
                }
                $UsersWithMissingAttributes += $UserWithMissing
            }
        }
        
        if ($UsersWithMissingAttributes) {
            Write-ADReportLog -Message "$($UsersWithMissingAttributes.Count) Benutzer mit fehlenden erforderlichen Attributen gefunden." -Type Info -GUI
            return $UsersWithMissingAttributes
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit fehlenden erforderlichen Attributen gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Benutzer mit fehlenden Attributen' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit doppelten Anmeldenamen (Quick Report) ---
Function Get-ADReportUsersDuplicateLogonNames {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Benutzer mit doppelten Anmeldenamen' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "DistinguishedName", "Created", "Modified", 
            "Description", "mail", "ObjectClass", "UserPrincipalName"
        )
        
        # Alle Benutzer laden
        $AllUsers = Get-ADUser -Filter * -Properties $Properties -ErrorAction SilentlyContinue
        
        # Gruppiere nach SamAccountName und finde Duplikate
        $DuplicateSamAccounts = $AllUsers | Group-Object SamAccountName | Where-Object { $_.Count -gt 1 }
        
        # Gruppiere nach UserPrincipalName und finde Duplikate
        ...
        foreach ($Group in $DuplicateSamAccounts) {
            foreach ($User in $Group.Group) {
                $DuplicateUser = $User | Select-Object *, @{
                    Name='DuplicateType'; Expression={'SamAccountName'}
                }, @{
                    Name='DuplicateValue'; Expression={$User.SamAccountName}
                }, @{
                    Name='DuplicateCount'; Expression={$Group.Count}
                }, @{
                    Name='ConflictWith'; Expression={
                        ($Group.Group | Where-Object { $_.DistinguishedName -ne $User.DistinguishedName } | 
                         ForEach-Object { $_.DistinguishedName }) -join '; '
                    }
                }
        ...
        # Verarbeite UserPrincipalName-Duplikate
        foreach ($Group in $DuplicateUPNs) {
            foreach ($User in $Group.Group) {
                # Prüfe, ob dieser Benutzer nicht bereits wegen SamAccountName-Duplikat erfasst wurde
                $AlreadyAdded = $DuplicateUsers | Where-Object { $_.DistinguishedName -eq $User.DistinguishedName -and $_.DuplicateType -eq 'SamAccountName' }
                
                if (-not $AlreadyAdded) {
                    $DuplicateUser = $User | Select-Object *, @{
                        Name='DuplicateType'; Expression={'UserPrincipalName'}
                    }, @{
                        Name='DuplicateValue'; Expression={$User.UserPrincipalName}
                    }, @{
                        Name='DuplicateCount'; Expression={$Group.Count}
                    }, @{
                        Name='ConflictWith'; Expression={
                    ...
                    # Aktualisiere den vorhandenen Eintrag um UPN-Duplikat-Info
                    $ExistingUser = $DuplicateUsers | Where-Object { $_.DistinguishedName -eq $User.DistinguishedName }
                    if ($ExistingUser) {
                        $ExistingUser.DuplicateType = "SamAccountName + UserPrincipalName"
                    }
                }
            }
        }
        ...
        if ($DuplicateUsers) {
            Write-ADReportLog -Message "$($DuplicateUsers.Count) Benutzer mit doppelten Anmeldenamen gefunden." -Type Info -GUI
            return $DuplicateUsers | Sort-Object DuplicateType, SamAccountName
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit doppelten Anmeldenamen gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Benutzer mit doppelten Anmeldenamen' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}
...
# --- Funktion zum Abrufen von Benutzern mit bald ablaufenden Passwörtern (Quick Report) ---
Function Get-ADReportPasswordExpiringSoon {
    [CmdletBinding()]
    param(
        [int]$Days = 7 # Standardmäßig 7 Tage
    )

    Write-ADReportLog -Message "Lade 'Passwörter laufen bald ab (nächste $Days Tage)' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "Enabled", "PasswordLastSet", 
            "PasswordNeverExpires", "mail", "Department", "Title", "Manager",
            "DistinguishedName", "ObjectClass", "msDS-UserPasswordExpiryTimeComputed"
        )
        
        # Hole die Domain-Passwort-Policy
        $DomainPolicy = Get-ADDefaultDomainPasswordPolicy -ErrorAction SilentlyContinue
        $MaxPasswordAge = if ($DomainPolicy) { $DomainPolicy.MaxPasswordAge.Days } else { 90 }
        
        # Berechne den Datumsbereich
        $Today = Get-Date
        $FutureDate = $Today.AddDays($Days)
        
        # Finde Benutzer mit ablaufenden Passwörtern
        ...
        foreach ($User in $Users) {
            if ($User.'msDS-UserPasswordExpiryTimeComputed' -ne $null) {
                # Verwende den berechneten Wert
                $ExpiryDate = [DateTime]::FromFileTime($User.'msDS-UserPasswordExpiryTimeComputed')
            } elseif ($User.PasswordLastSet -ne $null) {
                # Fallback: Berechne basierend auf PasswordLastSet
            ...
            # Füge nur Benutzer hinzu, deren Passwort in den nächsten X Tagen abläuft
            if ($DaysUntilExpiry -ge 0 -and $DaysUntilExpiry -le $Days) {
                $User | Add-Member -MemberType NoteProperty -Name "PasswordExpiryDate" -Value $ExpiryDate -Force
                $User | Add-Member -MemberType NoteProperty -Name "DaysUntilExpiry" -Value $DaysUntilExpiry -Force
                $ExpiringUsers += $User
            }
        }
        
        if ($ExpiringUsers) {
            Write-ADReportLog -Message "$($ExpiringUsers.Count) Benutzer mit in den nächsten $Days Tagen ablaufenden Passwörtern gefunden." -Type Info -GUI
            return $ExpiringUsers | Select-Object $Properties, PasswordExpiryDate, DaysUntilExpiry | 
                   Sort-Object DaysUntilExpiry
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit in den nächsten $Days Tagen ablaufenden Passwörtern gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Passwörter laufen bald ab' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit abgelaufenen Passwörtern (Quick Report) ---
Function Get-ADReportExpiredPasswords {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Abgelaufene Passwörter' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "Enabled", "PasswordLastSet", 
            "PasswordNeverExpires", "LastLogonDate", "mail", "Department", "Title", 
            "Manager", "DistinguishedName", "ObjectClass", "msDS-UserPasswordExpiryTimeComputed"
        )
        ...
        # Finde Benutzer mit abgelaufenen Passwörtern
        $Users = Get-ADUser -Filter { 
            (Enabled -eq $true) -and 
            (PasswordNeverExpires -eq $false) -and 
            (PasswordLastSet -ne $null) 
        ...
        foreach ($User in $Users) {
            ...
            # Berechne Tage seit Ablauf
            $DaysSinceExpiry = [Math]::Round(($Today - $ExpiryDate).TotalDays, 0)
            
            # Füge nur Benutzer hinzu, deren Passwort bereits abgelaufen ist
            if ($DaysSinceExpiry -gt 0) {
                $User | Add-Member -MemberType NoteProperty -Name "PasswordExpiryDate" -Value $ExpiryDate -Force
                $User | Add-Member -MemberType NoteProperty -Name "DaysSinceExpiry" -Value $DaysSinceExpiry -Force
                $ExpiredUsers += $User
            }
        }
        
        if ($ExpiredUsers) {
            Write-ADReportLog -Message "$($ExpiredUsers.Count) Benutzer mit abgelaufenen Passwörtern gefunden." -Type Info -GUI
            return $ExpiredUsers | Select-Object $Properties, PasswordExpiryDate, DaysSinceExpiry | 
                   Sort-Object DaysSinceExpiry -Descending
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit abgelaufenen Passwörtern gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Abgelaufene Passwörter' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von verwaisten SIDs (Benutzer) (Quick Report) ---
Function Get-ADReportOrphanedSIDsUsers {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Verwaiste SIDs (Benutzer)' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "Name", "SamAccountName", "SID", "ObjectGUID", "whenCreated", 
            "whenChanged", "DistinguishedName", "Description", "ObjectClass"
        )
        
        # Suche nach Objekten mit Fremd-SIDs oder speziellen Bedingungen
        $Results = @()
        
        # 1. Suche nach ForeignSecurityPrincipals (Externe SIDs)
        $ForeignPrincipals = Get-ADObject -Filter { objectClass -eq "foreignSecurityPrincipal" } -Properties $Properties -ErrorAction SilentlyContinue
        
        foreach ($Principal in $ForeignPrincipals) {
            # Versuche die SID aufzulösen
            try {
                $null = [System.Security.Principal.SecurityIdentifier]::new($Principal.Name).Translate([System.Security.Principal.NTAccount])
                # Wenn erfolgreich aufgelöst, ist es kein verwaister SID
            } catch {
                # Konnte nicht aufgelöst werden = verwaist
                $Principal | Add-Member -MemberType NoteProperty -Name "OrphanType" -Value "ForeignSecurityPrincipal" -Force
                $Principal | Add-Member -MemberType NoteProperty -Name "Status" -Value "Unresolvable SID" -Force
                $Results += $Principal
            }
        }
        
        # 2. Suche nach Benutzern mit ungültigen primären Gruppen
        $Users = Get-ADUser -Filter * -Properties PrimaryGroupID, MemberOf -ErrorAction SilentlyContinue
        
        foreach ($User in $Users) {
            # Standard Primary Group ID für Domain Users ist 513
            if ($User.PrimaryGroupID -ne 513) {
                # Überprüfe ob die primäre Gruppe existiert
                $PrimaryGroupDN = (Get-ADDomain).DistinguishedName -replace '^', "CN=$($User.PrimaryGroupID),"
                try {
                    $null = Get-ADGroup -Identity $PrimaryGroupDN -ErrorAction Stop
                } catch {
                    # Primäre Gruppe existiert nicht
                    $UserInfo = Get-ADUser -Identity $User.SamAccountName -Properties $Properties -ErrorAction SilentlyContinue
                    if ($UserInfo) {
                        $UserInfo | Add-Member -MemberType NoteProperty -Name "OrphanType" -Value "InvalidPrimaryGroup" -Force
                        $UserInfo | Add-Member -MemberType NoteProperty -Name "Status" -Value "Primary Group ID: $($User.PrimaryGroupID) not found" -Force
                        $Results += $UserInfo
                    }
                }
            }
        ...
        # 3. Suche nach Well-Known SIDs die nicht mehr gültig sind
        $WellKnownSIDs = @(
            @{Name="Guest"; SID="S-1-5-21-*-501"},
            @{Name="Administrator"; SID="S-1-5-21-*-500"},
            @{Name="krbtgt"; SID="S-1-5-21-*-502"}
        )
        
        foreach ($WKS in $WellKnownSIDs) {
            try {
                $Account = Get-ADUser -Filter "SamAccountName -eq '$($WKS.Name)'" -Properties $Properties -ErrorAction SilentlyContinue
                if (-not $Account) {
                    # Well-Known Account fehlt
                    $OrphanedWKS = [PSCustomObject]@{
                        Name = $WKS.Name
                        SamAccountName = $WKS.Name
                        SID = $WKS.SID
                        OrphanType = "MissingWellKnownAccount"
                        Status = "Well-Known Account not found"
                        ObjectClass = "user"
                    }
                    $Results += $OrphanedWKS
                }
        ...
        if ($Results) {
            Write-ADReportLog -Message "$($Results.Count) verwaiste SIDs gefunden." -Type Info -GUI
            return $Results | Select-Object Name, SamAccountName, SID, OrphanType, Status, whenCreated, whenChanged, DistinguishedName, ObjectClass
        } else {
            Write-ADReportLog -Message "Keine verwaisten SIDs gefunden." -Type Warning -GUI
            return @()
        }
    }
    catch {
        $ErrorMessage = "Fehler beim Laden des 'Verwaiste SIDs' Reports: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}
...
# --- Funktion zum Abrufen von Benutzern, die sich noch nie angemeldet haben ---
Function Get-ADReportNeverLoggedOnUsers {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Nie angemeldete Benutzer' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "whenCreated", "PasswordLastSet", "DistinguishedName", "mail", "ObjectClass"
        )
        
        # Benutzer ohne LastLogonDate oder LastLogon oder LastLogonTimestamp abrufen
        $AllUsers = Get-ADUser -Filter * -Properties $Properties -ErrorAction Stop
        
        $NeverLoggedOnUsers = $AllUsers | Where-Object {
            # Mehrere Attribute prüfen, da LastLogonDate allein nicht immer zuverlässig ist
            $null -eq $_.LastLogonDate -and 
            (
                # Nur Benutzer, die älter als 7 Tage sind (um neu erstellte Konten zu ignorieren)
                $_.whenCreated -lt (Get-Date).AddDays(-7)
            )
        } | Select-Object $Properties, 
            @{Name="AccountAge"; Expression={
                [math]::Round((New-TimeSpan -Start $_.whenCreated -End (Get-Date)).TotalDays)
            }},
            @{Name="NeverLoggedOn"; Expression={"True"}}
        
        if ($NeverLoggedOnUsers) {
            Write-ADReportLog -Message "$($NeverLoggedOnUsers.Count) Benutzer gefunden, die sich noch nie angemeldet haben." -Type Info -GUI
            return $NeverLoggedOnUsers
        } else {
            Write-ADReportLog -Message "Keine Benutzer gefunden, die sich noch nie angemeldet haben." -Type Info -GUI
            return @()
        }
    }
    catch {
...
# --- Funktion zum Abrufen kürzlich gelöschter Benutzer ---
Function Get-ADReportRecentlyDeletedUsers {
    [CmdletBinding()]
    param(
    ...
    Write-ADReportLog -Message "Lade 'Kürzlich gelöschte Benutzer (letzte $($DaysAgo) Tage)' Report..." -Type Info -GUI
    try {
        # Active Directory Recycle Bin muss aktiviert sein
        # Prüfen, ob AD Recycle Bin aktiviert ist
        ...
        try {
            ...
            if ($RecycleBinFeature) {
                $IsRecycleBinEnabled = ($RecycleBinFeature.EnabledScopes -contains $ADForest.PartitionsContainer)
            }
        } catch {
            $IsRecycleBinEnabled = $false
        }
        
        if (-not $IsRecycleBinEnabled) {
            Write-ADReportLog -Message "Der Active Directory-Papierkorb ist nicht aktiviert. Die Abfrage gelöschter Benutzer ist nicht möglich." -Type Warning -GUI
            return @(
                [PSCustomObject]@{
                    DisplayName = "AD Recycle Bin nicht aktiviert"
                    SamAccountName = "N/A"
                    DeletionTime = (Get-Date)
                    OriginalOU = "Um gelöschte Objekte zu sehen, aktivieren Sie den AD Recycle Bin."
                    ObjectClass = "Information"
                }
            )
        }
        
        # Datum für Filter setzen
        $cutoffDate = (Get-Date).AddDays(-$DaysAgo)
        
        # Nach gelöschten Benutzern suchen
        $DeletedUsers = Get-ADObject -Filter {
            (objectClass -eq "user") -and (isDeleted -eq $true) -and (whenChanged -ge $cutoffDate)
        } -IncludeDeletedObjects -Properties DisplayName, SamAccountName, whenChanged, lastKnownParent -ErrorAction SilentlyContinue
        
        if ($DeletedUsers) {
            $FormattedResults = $DeletedUsers | Select-Object DisplayName, SamAccountName, 
                @{Name="DeletionTime"; Expression={$_.whenChanged}},
                @{Name="OriginalOU"; Expression={$_.lastKnownParent}},
                @{Name="ObjectClass"; Expression={"user"}}
            
            Write-ADReportLog -Message "$($FormattedResults.Count) kürzlich gelöschte Benutzer gefunden." -Type Info -GUI
            return $FormattedResults
        } else {
            Write-ADReportLog -Message "Keine kürzlich gelöschten Benutzer gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen kürzlich gelöschter Benutzer: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen kürzlich modifizierter Benutzer ---
Function Get-ADReportRecentlyModifiedUsers {
    [CmdletBinding()]
    param(
        [int]$DaysAgo = 7 # Standardmäßig die letzten 7 Tage
    )

    Write-ADReportLog -Message "Lade 'Kürzlich geänderte Benutzer (letzte $($DaysAgo) Tage)' Report..." -Type Info -GUI
    try {
        $cutoffDate = (Get-Date).AddDays(-$DaysAgo)
        
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "whenCreated", "whenChanged", "Modified", "PasswordLastSet", 
            "DistinguishedName", "mail", "ObjectClass"
        )
        
        # Suche nach Benutzern, die innerhalb des angegebenen Zeitraums geändert wurden
        $ModifiedUsers = Get-ADUser -Filter {whenChanged -ge $cutoffDate} -Properties $Properties -ErrorAction Stop
        
        if ($ModifiedUsers) {
            $FormattedResults = $ModifiedUsers | Select-Object $Properties,
                @{Name="ModifiedDaysAgo"; Expression={
                    [math]::Round((New-TimeSpan -Start $_.whenChanged -End (Get-Date)).TotalDays, 1)
                }}
            
            Write-ADReportLog -Message "$($FormattedResults.Count) kürzlich geänderte Benutzer gefunden." -Type Info -GUI
            return $FormattedResults
        } else {
            Write-ADReportLog -Message "Keine kürzlich geänderten Benutzer gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen kürzlich geänderter Benutzer: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen inaktiver Benutzer (mit benutzerdefiniertem Zeitraum) ---
Function Get-ADReportInactiveUsersXDays {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Inaktive Benutzer (X Tage)' Report..." -Type Info -GUI
    try {
        # Dialog zur Eingabe der Tagesanzahl anzeigen
        $form = New-Object System.Windows.Forms.Form
        $form.Text = "Inaktivitätszeitraum"
        ...
        $label.Text = "Bitte geben Sie die Anzahl der Tage ein:"
        $form.Controls.Add($label)

        $textBox = New-Object System.Windows.Forms.TextBox
        $textBox.Location = New-Object System.Drawing.Point(10,50)
        $textBox.Size = New-Object System.Drawing.Size(260,20)
        $textBox.Text = "90" # Standardwert
        ...
        $cancelButton.Location = New-Object System.Drawing.Point(155,80)
        ...
        if ($result -ne [System.Windows.Forms.DialogResult]::OK) {
            Write-ADReportLog -Message "Abfrage abgebrochen." -Type Info -GUI
            return $null
        }

        # Eingabe validieren
        $days = 0
        if (-not [int]::TryParse($textBox.Text, [ref]$days) -or $days -le 0) {
            Write-ADReportLog -Message "Ungültige Eingabe. Bitte geben Sie eine positive Zahl ein." -Type Warning -GUI
            return $null
        }

        Write-ADReportLog -Message "Suche nach Benutzern, die seit $days Tagen inaktiv sind..." -Type Info -GUI

        $cutoffDate = (Get-Date).AddDays(-$days)
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "whenCreated", "PasswordLastSet", "DistinguishedName", 
            "mail", "ObjectClass"
        )

        # Benutzer abrufen, die seit dem angegebenen Zeitraum inaktiv sind
        ...
        if ($InactiveUsers) {
            Write-ADReportLog -Message "$($InactiveUsers.Count) inaktive Benutzer (>${days} Tage) gefunden." -Type Info -GUI
            return $InactiveUsers
        } else {
            Write-ADReportLog -Message "Keine inaktiven Benutzer (>${days} Tage) gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen inaktiver Benutzer: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern ohne Manager ---
Function Get-ADReportUsersWithoutManager {
    [CmdletBinding()]
    param()
    ...
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "Manager", "Department", "Title", "Company", 
            "DistinguishedName", "mail", "ObjectClass"
        )
        ...
        # Benutzer ohne Manager filtern
        $UsersWithoutManager = $AllUsers | Where-Object { 
            $null -eq $_.Manager -or $_.Manager -eq "" 
        } | Select-Object $Properties, 
            @{Name="ManagerMissing"; Expression={"True"}}
        
        if ($UsersWithoutManager) {
            Write-ADReportLog -Message "$($UsersWithoutManager.Count) Benutzer ohne Manager gefunden." -Type Info -GUI
            return $UsersWithoutManager
        } else {
            Write-ADReportLog -Message "Keine Benutzer ohne Manager gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von Benutzern ohne Manager: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit fehlenden erforderlichen Attributen ---
Function Get-ADReportUsersMissingRequiredAttributes {
    [CmdletBinding()]
    param()
    ...
    Write-ADReportLog -Message "Lade 'Benutzer mit fehlenden erforderlichen Attributen' Report..." -Type Info -GUI
    try {
        # Definition der Pflichtattribute für einen vollständigen Benutzer
        $RequiredAttributes = @(
            "DisplayName", "GivenName", "Surname", "mail", 
            "Department", "Title", "Company", "telephoneNumber"
        )
        
        $Properties = $RequiredAttributes + @(
            "SamAccountName", "Enabled", "LastLogonDate", "DistinguishedName", "ObjectClass"
        )
        ...
        foreach ($User in $AllUsers) {
            ...
            foreach ($Attr in $RequiredAttributes) {
                if ([string]::IsNullOrWhiteSpace($User.$Attr)) {
                    ...
                    # Schweregrad-Score basierend auf der Wichtigkeit des Attributs
                    switch ($Attr) {
                        "DisplayName" { $SeverityScore += 5 }
                        "GivenName" { $SeverityScore += 4 }
                        "Surname" { $SeverityScore += 4 }
                        "mail" { $SeverityScore += 5 }
                        "Department" { $SeverityScore += 3 }
                        "Title" { $SeverityScore += 3 }
                        "Company" { $SeverityScore += 3 }
                        "telephoneNumber" { $SeverityScore += 2 }
                    }
                }
            }
            ...
            if ($MissingAttributes.Count -gt 0) {
                $UsersWithMissingAttributes += [PSCustomObject]@{
                    ...
                    SeverityRating = if ($SeverityScore -ge 10) { "Hoch" } elseif ($SeverityScore -ge 5) { "Mittel" } else { "Niedrig" }
        ...
        if ($UsersWithMissingAttributes.Count -gt 0) {
            # Nach Schweregrad und dann nach Anzahl fehlender Attribute sortieren
            $SortedResults = $UsersWithMissingAttributes | Sort-Object -Property SeverityScore, MissingCount -Descending
            
            Write-ADReportLog -Message "$($SortedResults.Count) Benutzer mit fehlenden erforderlichen Attributen gefunden." -Type Info -GUI
            return $SortedResults
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit fehlenden erforderlichen Attributen gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von Benutzern mit fehlenden Attributen: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit doppelten Anmeldenamen ---
Function Get-ADReportUsersDuplicateLogonNames {
    [CmdletBinding()]
    param()
    ...
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "UserPrincipalName", "GivenName", "Surname", 
            "Enabled", "LastLogonDate", "DistinguishedName", "mail", "ObjectClass"
        )
        
        # Alle Benutzer abrufen
        $AllUsers = Get-ADUser -Filter * -Properties $Properties -ErrorAction Stop
        
        # Gruppiere nach SamAccountName und prüfe auf Duplikate
        $SamAccountGroups = $AllUsers | Group-Object -Property SamAccountName | Where-Object { $_.Count -gt 1 }
        
        # Gruppiere nach UserPrincipalName und prüfe auf Duplikate
        $UpnGroups = $AllUsers | Group-Object -Property UserPrincipalName | Where-Object { $_.Count -gt 1 }
        
        # Sammle alle Duplikate
        $DuplicateUsers = @()
        
        if ($SamAccountGroups) {
            foreach ($Group in $SamAccountGroups) {
                foreach ($User in $Group.Group) {
        ...
        if ($UpnGroups) {
            foreach ($Group in $UpnGroups) {
                foreach ($User in $Group.Group) {
                    # Prüfen, ob der Benutzer bereits durch SamAccountName-Duplikat hinzugefügt wurde
                    $existingUser = $DuplicateUsers | Where-Object { $_.DistinguishedName -eq $User.DistinguishedName }
                    
                    if ($existingUser) {
                        # Aktualisiere den DuplicateType für bestehende Einträge
                        $existingUser.DuplicateType = "SamAccountName & UserPrincipalName"
                    } else {
        ...
        if ($DuplicateUsers.Count -gt 0) {
            ...
            Write-ADReportLog -Message "Keine Benutzer mit doppelten Anmeldenamen gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von Benutzern mit doppelten Anmeldenamen: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit bald ablaufenden Passwörtern ---
Function Get-ADReportPasswordExpiringSoon {
    [CmdletBinding()]
    param(
        [int]$Days = 7 # Standardmäßig die nächsten 7 Tage
    )

    Write-ADReportLog -Message "Lade 'Passwörter laufen bald ab (${Days} Tage)' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "PasswordLastSet", "PasswordNeverExpires", "PasswordExpired", 
            "msDS-UserPasswordExpiryTimeComputed", "DistinguishedName", "mail", "ObjectClass"
        )
        
        # Aktive Benutzer abrufen, deren Passwörter ablaufen können
        ...
        foreach ($User in $AllUsers) {
            if ($User."msDS-UserPasswordExpiryTimeComputed" -gt 0) {
                ...
                if ($ExpiryTime -gt $Today -and $ExpiryTime -lt $ExpiryDate) {
                    $UsersWithExpiringPasswords += [PSCustomObject]@{
                        DisplayName = $User.DisplayName
                        SamAccountName = $User.SamAccountName
                        GivenName = $User.GivenName
                        Surname = $User.Surname
                        ...
                        LastLogonDate = $User.LastLogonDate
                        PasswordLastSet = $User.PasswordLastSet
                        PasswordExpiryDate = $ExpiryTime
                        DaysUntilExpiry = [math]::Round(($ExpiryTime - $Today).TotalDays, 1)
        ...
        if ($UsersWithExpiringPasswords.Count -gt 0) {
            ...
            $SortedResults = $UsersWithExpiringPasswords | Sort-Object -Property DaysUntilExpiry
            
            Write-ADReportLog -Message "$($SortedResults.Count) Benutzer mit bald ablaufenden Passwörtern gefunden." -Type Info -GUI
            return $SortedResults
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit bald ablaufenden Passwörtern gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von Benutzern mit bald ablaufenden Passwörtern: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von Benutzern mit abgelaufenen Passwörtern ---
Function Get-ADReportExpiredPasswords {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Lade 'Benutzer mit abgelaufenen Passwörtern' Report..." -Type Info -GUI
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "GivenName", "Surname", "Enabled", 
            "LastLogonDate", "PasswordLastSet", "PasswordNeverExpires", "PasswordExpired", 
        ...
        # Aktive Benutzer mit abgelaufenen Passwörtern abrufen
        $ExpiredPasswordUsers = Get-ADUser -Filter {
            Enabled -eq $true -and PasswordExpired -eq $true
        } -Properties $Properties -ErrorAction Stop
        
        if ($ExpiredPasswordUsers) {
            $Today = Get-Date
            
            $FormattedResults = $ExpiredPasswordUsers | Select-Object $Properties,
                @{Name="PasswordAge"; Expression={
                    if ($_.PasswordLastSet) {
                        [math]::Round(($Today - $_.PasswordLastSet).TotalDays, 1)
                    } else { "N/A" }
                }}
            
            Write-ADReportLog -Message "$($FormattedResults.Count) Benutzer mit abgelaufenen Passwörtern gefunden." -Type Info -GUI
            return $FormattedResults
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit abgelaufenen Passwörtern gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von Benutzern mit abgelaufenen Passwörtern: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}

# --- Funktion zum Abrufen von verwaisten SIDs in Benutzerkonten ---
Function Get-ADReportOrphanedSIDsUsers {
    [CmdletBinding()]
    param()
    ...
    try {
        $Properties = @(
            "DisplayName", "SamAccountName", "SIDHistory", "Enabled", 
            "LastLogonDate", "whenCreated", "DistinguishedName", "ObjectClass"
        )
        
        # Benutzer mit SIDHistory abrufen
        $UsersWithSIDHistory = Get-ADUser -Filter {SIDHistory -like "*"} -Properties $Properties -ErrorAction Stop
        ...
        if ($UsersWithSIDHistory) {
            ...
            foreach ($User in $UsersWithSIDHistory) {
                foreach ($SID in $User.SIDHistory) {
                    # Versuche, ein Objekt mit dieser SID zu finden
                    $SourceObject = $null
                    try {
                        $SourceObject = Get-ADObject -Filter {ObjectSID -eq $SID} -Properties ObjectClass, Name -ErrorAction SilentlyContinue
                    } catch {
                        # Ignoriere Fehler - nicht gefundene SIDs sind genau das, was wir suchen
                    }
                    ...
                    $FormattedResults += [PSCustomObject]@{
                        ...
                        Enabled = $User.Enabled
                        LastLogonDate = $User.LastLogonDate
                        SIDHistoryValue = $SID.Value
                        IsOrphaned = ($null -eq $SourceObject)
                        SourceObjectExists = ($null -ne $SourceObject)
                        SourceObjectName = if ($SourceObject) { $SourceObject.Name } else { "N/A" }
                        SourceObjectClass = if ($SourceObject) { $SourceObject.ObjectClass } else { "N/A" }
                        AccountCreated = $User.whenCreated
                        ObjectClass = $User.ObjectClass
                        DistinguishedName = $User.DistinguishedName
                    }
            ...
            # Filtere nur wirklich verwaiste SIDs
            $OrphanedSIDs = $FormattedResults | Where-Object { $_.IsOrphaned }
            
            if ($OrphanedSIDs) {
                Write-ADReportLog -Message "$($OrphanedSIDs.Count) verwaiste SIDs in Benutzerkonten gefunden." -Type Info -GUI
                return $OrphanedSIDs
            } else {
                Write-ADReportLog -Message "Keine verwaisten SIDs in Benutzerkonten gefunden." -Type Info -GUI
                return @()
            }
        } else {
            Write-ADReportLog -Message "Keine Benutzer mit SID-History gefunden." -Type Info -GUI
            return @()
        }
    } catch {
        $ErrorMessage = "Fehler beim Abrufen von verwaisten SIDs: $($_.Exception.Message)"
        Write-ADReportLog -Message $ErrorMessage -Type Error -GUI
        return $null
    }
}
...
# Funktion zum Aktualisieren der Ergebniszähler im Header
    ...
    Function Update-ResultCounters {
        ...
        try {
            # Sichere Konvertierung zu Array
            $ResultsArray = @()
            if ($null -ne $Results) {
                if ($Results -is [Array]) {
            ...
            Write-Warning "Error in Update-ResultCounters parameter processing: $($_.Exception.Message)"
            if ($null -ne $Global:TotalResultCountText) { 
    ...
    # Sichere Umwandlung der Results in ein Array
    $ResultsCollection = @()
    if ($null -ne $Results) {
        if ($Results -is [Array]) {
    ...
    if ($null -ne $Global:DataGridResults) {
        $Global:DataGridResults.ItemsSource = $ResultsCollection
    }
    ...
    # Sichere Übergabe an Update-Funktionen
    try {
        Update-ResultCounters -Results $ResultsCollection
    } catch {
        Write-Warning "Error updating result counters: $($_.Exception.Message)"
    }
    
    try {
        Update-ResultVisualization -Results $ResultsCollection
    } catch {
        Write-Warning "Error updating result visualization: $($_.Exception.Message)"
    }
} 

# --- Globale Variablen für UI Elemente --- 
#region Quick Report Functions - Users
    Function Get-ADAllUsersReport {
        Write-Host "DEBUG: Get-ADAllUsersReport called"
        Write-ADReportLog -Message "Abrufen aller Benutzer..." -Type Info -GUI
        try {
            # TODO: Implement actual logic - this is a placeholder
            # Example: Get-ADUser -Filter * -Properties $Global:DefaultProperties | Select-Object $Global:DefaultProperties
            $ReportData = @() 
            if ($ReportData.Count -eq 0) {
                Write-ADReportLog -Message "Keine Benutzer gefunden oder Funktion noch nicht implementiert." -Type Warning -GUI
            } else {
                Write-ADReportLog -Message "Alle Benutzer erfolgreich abgerufen." -Type Success -GUI
            }
            return $ReportData
        } catch {
            Write-ADReportLog -Message "Fehler beim Abrufen aller Benutzer: $($_.Exception.Message)" -Type Error -GUI
    ...
    Function Get-ADDisabledUsersReport {
        Write-Host "DEBUG: Get-ADDisabledUsersReport called"
        Write-ADReportLog -Message "Abrufen deaktivierter Benutzer..." -Type Info -GUI
        try {
            # TODO: Implement actual logic
            # Example: Get-ADUser -Filter {Enabled -eq $false} -Properties $Global:DefaultProperties | Select-Object $Global:DefaultProperties
            $ReportData = @()
            if ($ReportData.Count -eq 0) {
                Write-ADReportLog -Message "Keine deaktivierten Benutzer gefunden oder Funktion noch nicht implementiert." -Type Warning -GUI
            } else {
                Write-ADReportLog -Message "Deaktivierte Benutzer erfolgreich abgerufen." -Type Success -GUI
            }
            return $ReportData
        } catch {
            Write-ADReportLog -Message "Fehler beim Abrufen deaktivierter Benutzer: $($_.Exception.Message)" -Type Error -GUI
    ...
    Function Get-ADLockedUsersReport {
        Write-Host "DEBUG: Get-ADLockedUsersReport called"
        Write-ADReportLog -Message "Abrufen gesperrter Benutzer..." -Type Info -GUI
        try {
            # TODO: Implement actual logic
            # Example: Search-ADAccount -LockedOut | Get-ADUser -Properties $Global:DefaultProperties | Select-Object $Global:DefaultProperties
            $ReportData = @()
            if ($ReportData.Count -eq 0) {
                Write-ADReportLog -Message "Keine gesperrten Benutzer gefunden oder Funktion noch nicht implementiert." -Type Warning -GUI
            } else {
                Write-ADReportLog -Message "Gesperrte Benutzer erfolgreich abgerufen." -Type Success -GUI
            }
            return $ReportData
        } catch {
            Write-ADReportLog -Message "Fehler beim Abrufen gesperrter Benutzer: $($_.Exception.Message)" -Type Error -GUI
    ...
    #endregion Quick Report Functions - Users

Function Initialize-ADReportForm {
    param($XAMLContent)
    
    Write-Host "Initialisiere easyADReport GUI..." -ForegroundColor Green
    ...
    # Sicherstellen, dass WPF-Assemblies geladen sind
    try {
    ...
    try {
        Write-Host "Lade XAML-Inhalt..." -ForegroundColor Yellow
        
        # XAML-String bereinigen und validieren
        $XAMLString = $XAMLContent.OuterXml
        Write-Host "XAML-String Länge: $($XAMLString.Length) Zeichen" -ForegroundColor Cyan
        
        # XAML mit StringReader laden (robuster als XmlNodeReader)
        ...
        if ($null -eq $Global:Window) {
            Write-Error "XAML-Window konnte nicht geladen werden - Ergebnis ist NULL"
            return $false
        }
        
        Write-Host "XAML erfolgreich geladen. Window-Typ: $($Global:Window.GetType().FullName)" -ForegroundColor Green
    ...
    Write-Host "Lade UI-Elemente aus XAML..." -ForegroundColor Yellow
    
    # Test ob Window-Objekt verfügbar ist
    if ($null -eq $Global:Window) {
        Write-Error "Window-Objekt ist NULL - kann UI-Elemente nicht laden"
        return $false
    }
    
    # Objekttyp Radio Buttons
    $Global:RadioButtonUser = $Global:Window.FindName("RadioButtonUser")
    $Global:RadioButtonGroup = $Global:Window.FindName("RadioButtonGroup")
    $Global:RadioButtonComputer = $Global:Window.FindName("RadioButtonComputer")
    $Global:RadioButtonGroupMemberships = $Global:Window.FindName("RadioButtonGroupMemberships")
    
    # Validierung der wichtigsten UI-Elemente
    $criticalElements = @{
        "RadioButtonUser" = $Global:RadioButtonUser
        "RadioButtonGroup" = $Global:RadioButtonGroup
        "RadioButtonComputer" = $Global:RadioButtonComputer
    }
    
    $missingElements = @()
    foreach ($elementName in $criticalElements.Keys) {
        if ($null -eq $criticalElements[$elementName]) {
            $missingElements += $elementName
        }
    }
    
    if ($missingElements.Count -gt 0) {
        Write-Error "Kritische UI-Elemente fehlen: $($missingElements -join ', ')"
        Write-Host "Verfügbare Child-Elemente im Window:" -ForegroundColor Yellow
        try {
            $allChildren = $Global:Window.FindName("*")
            if ($allChildren) {
                Write-Host "Gefundene Elemente: $($allChildren.Count)" -ForegroundColor Cyan
            } else {
                Write-Host "Keine Child-Elemente gefunden" -ForegroundColor Red
            }
        } catch {
            Write-Host "Fehler beim Auflisten der Child-Elemente: $($_.Exception.Message)" -ForegroundColor Red
        }
        return $false
    }

    # Filter und Attribute
    $Global:ComboBoxFilterAttribute = $Global:Window.FindName("ComboBoxFilterAttribute")
    $Global:TextBoxFilterValue = $Global:Window.FindName("TextBoxFilterValue")
    $Global:ListBoxSelectAttributes = $Global:Window.FindName("ListBoxSelectAttributes")
    $Global:ButtonQueryAD = $Global:Window.FindName("ButtonQueryAD")

    # Quick Reports - Benutzer
    $Global:ButtonQuickAllUsers = $Global:Window.FindName("ButtonQuickAllUsers")
    $Global:ButtonQuickDisabledUsers = $Global:Window.FindName("ButtonQuickDisabledUsers")
    $Global:ButtonQuickLockedUsers = $Global:Window.FindName("ButtonQuickLockedUsers")
    $Global:ButtonQuickNeverExpire = $Global:Window.FindName("ButtonQuickNeverExpire")
    $Global:ButtonQuickInactiveUsers = $Global:Window.FindName("ButtonQuickInactiveUsers")
    $Global:ButtonQuickAdminUsers = $Global:Window.FindName("ButtonQuickAdminUsers")
    $Global:ButtonQuickRecentlyCreatedUsers = $Global:Window.FindName("ButtonQuickRecentlyCreatedUsers")
    $Global:ButtonQuickPasswordExpiringSoon = $Global:Window.FindName("ButtonQuickPasswordExpiringSoon")
    $Global:ButtonQuickExpiredPasswords = $Global:Window.FindName("ButtonQuickExpiredPasswords")
    $Global:ButtonQuickNeverLoggedOn = $Global:Window.FindName("ButtonQuickNeverLoggedOn")
    
    # Quick Reports - Gruppen
    $Global:ButtonQuickGroups = $Global:Window.FindName("ButtonQuickGroups")
    $Global:ButtonQuickSecurityGroups = $Global:Window.FindName("ButtonQuickSecurityGroups")
    $Global:ButtonQuickDistributionGroups = $Global:Window.FindName("ButtonQuickDistributionGroups")
    
    # Quick Reports - Computer
    $Global:ButtonQuickComputers = $Global:Window.FindName("ButtonQuickComputers")
    $Global:ButtonQuickInactiveComputers = $Global:Window.FindName("ButtonQuickInactiveComputers")
    
    # Security Audit
    $Global:ButtonQuickWeakPasswordPolicy = $Global:Window.FindName("ButtonQuickWeakPasswordPolicy")
    $Global:ButtonQuickRiskyGroupMemberships = $Global:Window.FindName("ButtonQuickRiskyGroupMemberships")
    $Global:ButtonQuickPrivilegedAccounts = $Global:Window.FindName("ButtonQuickPrivilegedAccounts")
    
    # AD-Health
    $Global:ButtonQuickFSMORoles = $Global:Window.FindName("ButtonQuickFSMORoles")
    $Global:ButtonQuickDCStatus = $Global:Window.FindName("ButtonQuickDCStatus")
    $Global:ButtonQuickReplicationStatus = $Global:Window.FindName("ButtonQuickReplicationStatus")
    
    # OU & Topology Reports
    $Global:ButtonQuickOUHierarchy = $Global:Window.FindName("ButtonQuickOUHierarchy")
    $Global:ButtonQuickSitesSubnets = $Global:Window.FindName("ButtonQuickSitesSubnets")

    # Ergebnisanzahl-Anzeige
    $Global:ResultCountGrid = $Global:Window.FindName("ResultCountGrid")
    $Global:TotalResultCountText = $Global:Window.FindName("TotalResultCountText")
    
    # Ergebnisse
    $Global:DataGridResults = $Global:Window.FindName("DataGridResults")

    # Status und Export
    $Global:TextBlockStatus = $Global:Window.FindName("TextBlockStatus")
    $Global:ButtonExportCSV = $Global:Window.FindName("ButtonExportCSV")
    $Global:ButtonExportHTML = $Global:Window.FindName("ButtonExportHTML")
    $Global:ButtonClose = $Global:Window.FindName("ButtonClose")
    $Global:ButtonClose = $Global:Window.FindName("ButtonClose")

    # Weitere fehlende Button-Referenzen
    $Global:ButtonQuickRecentlyDeletedUsers = $Global:Window.FindName("ButtonQuickRecentlyDeletedUsers")
    $Global:ButtonQuickRecentlyModifiedUsers = $Global:Window.FindName("ButtonQuickRecentlyModifiedUsers")
    $Global:ButtonQuickInactiveUsersXDays = $Global:Window.FindName("ButtonQuickInactiveUsersXDays")
    $Global:ButtonQuickUsersWithoutManager = $Global:Window.FindName("ButtonQuickUsersWithoutManager")
    $Global:ButtonQuickUsersMissingRequiredAttributes = $Global:Window.FindName("ButtonQuickUsersMissingRequiredAttributes")
    $Global:ButtonQuickUsersDuplicateLogonNames = $Global:Window.FindName("ButtonQuickUsersDuplicateLogonNames")
    $Global:ButtonQuickOrphanedSIDsUsers = $Global:Window.FindName("ButtonQuickOrphanedSIDsUsers")
    $Global:ButtonQuickPasswordExpiringSoon = $Global:Window.FindName("ButtonQuickPasswordExpiringSoon")
    $Global:ButtonQuickExpiredPasswords = $Global:Window.FindName("ButtonQuickExpiredPasswords")
    $Global:ButtonQuickNeverLoggedOn = $Global:Window.FindName("ButtonQuickNeverLoggedOn")
    $Global:ButtonQuickRecentlyCreatedGroups = $Global:Window.FindName("ButtonQuickRecentlyCreatedGroups")
    $Global:ButtonQuickRecentlyDeletedGroups = $Global:Window.FindName("ButtonQuickRecentlyDeletedGroups")
    $Global:ButtonQuickEmptyGroups = $Global:Window.FindName("ButtonQuickEmptyGroups")
    $Global:ButtonQuickVeryLargeGroupsXMembers = $Global:Window.FindName("ButtonQuickVeryLargeGroupsXMembers")
    $Global:ButtonQuickDeeplyNestedGroupsXDepth = $Global:Window.FindName("ButtonQuickDeeplyNestedGroupsXDepth")
    $Global:ButtonQuickGroupsWithoutOwner = $Global:Window.FindName("ButtonQuickGroupsWithoutOwner")
    $Global:ButtonQuickGroupsWithoutDescription = $Global:Window.FindName("ButtonQuickGroupsWithoutDescription")
    $Global:ButtonQuickDistListsWithoutMailbox = $Global:Window.FindName("ButtonQuickDistListsWithoutMailbox")
    $Global:ButtonQuickGroupNestingInfo = $Global:Window.FindName("ButtonQuickGroupNestingInfo")
    
    Write-Host "UI-Elemente erfolgreich geladen." -ForegroundColor Green

    # Event Handler für Quick Reports (OU & Topology)
    if ($null -ne $Global:ButtonQuickRecentlyCreatedUsers) {
        $null = $Global:ButtonQuickRecentlyCreatedUsers.Add_Click({
    ...
    if ($null -ne $Global:ButtonClose) {
        $Global:ButtonClose.Add_Click({
            Write-Host "DEBUG: ButtonClose Clicked"
            Write-ADReportLog -Message "Exit Button geklickt. Anwendung wird beendet." -Type Info -GUI
            $Global:Window.Close()
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonClose' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickAllUsers
    if ($null -ne $Global:ButtonQuickAllUsers) {
        $null = $Global:ButtonQuickAllUsers.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickDisabledUsers) {
        $null = $Global:ButtonQuickDisabledUsers.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickLockedUsers) {
        $null = $Global:ButtonQuickLockedUsers.add_Click({
            Write-ADReportLog -Message "Button 'Gesperrte Benutzer' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADLockedUsersReport # Bestehende Funktion verwenden
            Update-ADReportResults -Results $ReportData 
        })
    } else {
    ...
    if ($null -ne $Global:ButtonQuickNeverExpire) {
        $null = $Global:ButtonQuickNeverExpire.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickInactiveUsers) {
        $null = $Global:ButtonQuickInactiveUsers.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickAdminUsers) {
        $null = $Global:ButtonQuickAdminUsers.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickRecentlyModifiedUsers) {
        $null = $Global:ButtonQuickRecentlyModifiedUsers.add_Click({
    ...
    # Event Handler für ButtonQuickNeverLoggedOn
    if ($null -ne $Global:ButtonQuickNeverLoggedOn) {
        $null = $Global:ButtonQuickNeverLoggedOn.add_Click({
            Write-ADReportLog -Message "Button 'Nie angemeldete Benutzer' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportNeverLoggedOnUsers
            Update-ADReportResults -Results $ReportData -StatusMessage "Nie angemeldete Benutzer Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickNeverLoggedOn' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickRecentlyDeletedUsers
    if ($null -ne $Global:ButtonQuickRecentlyDeletedUsers) {
        $null = $Global:ButtonQuickRecentlyDeletedUsers.add_Click({
            Write-ADReportLog -Message "Button 'Kürzlich gelöschte Benutzer' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportRecentlyDeletedUsers # Standardmäßig letzte 30 Tage
            Update-ADReportResults -Results $ReportData -StatusMessage "Kürzlich gelöschte Benutzer Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickRecentlyDeletedUsers' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickInactiveUsersXDays
    if ($null -ne $Global:ButtonQuickInactiveUsersXDays) {
        $null = $Global:ButtonQuickInactiveUsersXDays.add_Click({
            Write-ADReportLog -Message "Button 'Inaktive Benutzer (X Tage)' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportInactiveUsersXDays # Dialog für Tageseingabe wird in der Funktion angezeigt
            Update-ADReportResults -Results $ReportData -StatusMessage "Inaktive Benutzer (anpassbare Tage) Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickInactiveUsersXDays' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickUsersWithoutManager
    if ($null -ne $Global:ButtonQuickUsersWithoutManager) {
        $null = $Global:ButtonQuickUsersWithoutManager.add_Click({
            Write-ADReportLog -Message "Button 'Benutzer ohne Manager' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportUsersWithoutManager
            Update-ADReportResults -Results $ReportData -StatusMessage "Benutzer ohne Manager Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickUsersWithoutManager' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickUsersMissingRequiredAttributes
    if ($null -ne $Global:ButtonQuickUsersMissingRequiredAttributes) {
        $null = $Global:ButtonQuickUsersMissingRequiredAttributes.add_Click({
            Write-ADReportLog -Message "Button 'Benutzer mit fehlenden Attributen' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportUsersMissingRequiredAttributes
            Update-ADReportResults -Results $ReportData -StatusMessage "Benutzer mit fehlenden erforderlichen Attributen Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickUsersMissingRequiredAttributes' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickUsersDuplicateLogonNames
    if ($null -ne $Global:ButtonQuickUsersDuplicateLogonNames) {
        $null = $Global:ButtonQuickUsersDuplicateLogonNames.add_Click({
            Write-ADReportLog -Message "Button 'Benutzer mit doppelten Anmeldenamen' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportUsersDuplicateLogonNames
            Update-ADReportResults -Results $ReportData -StatusMessage "Benutzer mit doppelten Anmeldenamen Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickUsersDuplicateLogonNames' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickPasswordExpiringSoon
    if ($null -ne $Global:ButtonQuickPasswordExpiringSoon) {
        $null = $Global:ButtonQuickPasswordExpiringSoon.add_Click({
            Write-ADReportLog -Message "Button 'Password läuft bald ab (7 Tage)' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportPasswordExpiringSoon -Days 7 
            Update-ADReportResults -Results $ReportData -StatusMessage "Passwörter, die in den nächsten 7 Tagen ablaufen, geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickPasswordExpiringSoon' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickExpiredPasswords
    if ($null -ne $Global:ButtonQuickExpiredPasswords) {
        $null = $Global:ButtonQuickExpiredPasswords.add_Click({
            Write-ADReportLog -Message "Button 'Abgelaufene Passwörter' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportExpiredPasswords
            Update-ADReportResults -Results $ReportData -StatusMessage "Abgelaufene Passwörter Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickExpiredPasswords' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }

    # Event Handler für ButtonQuickOrphanedSIDsUsers
    if ($null -ne $Global:ButtonQuickOrphanedSIDsUsers) {
        $null = $Global:ButtonQuickOrphanedSIDsUsers.add_Click({
            Write-ADReportLog -Message "Button 'Verwaiste SIDs (Benutzer)' geklickt." -Type Info
            Initialize-ResultCounters # Zähler und UI zurücksetzen
            $ReportData = Get-ADReportOrphanedSIDsUsers
            Update-ADReportResults -Results $ReportData -StatusMessage "Verwaiste SIDs (Benutzer) Report geladen."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickOrphanedSIDsUsers' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }
    ...
    if ($null -ne $Global:ButtonQuickGroups) {
        $null = $Global:ButtonQuickGroups.add_Click({
    ...
    if ($null -ne $Global:ButtonQuickEmptyGroups) {
        $null = $Global:ButtonQuickEmptyGroups.add_Click({
    ...
    $null = $Global:ButtonQuickSitesSubnets.Add_Click({
    ...
    # Event Handler für ButtonQuickOUHierarchy
    if ($null -ne $Global:ButtonQuickOUHierarchy) {
        $null = $Global:ButtonQuickOUHierarchy.Add_Click({
            Write-ADReportLog -Message "Loading OU Hierarchy Report..." -Type Info -Terminal
            Initialize-ResultCounters
            $ReportData = Get-ADOUHierarchyReport
            Update-ADReportResults -Results $ReportData -StatusMessage "OU Hierarchy Report loaded."
        })
    } else {
        Write-ADReportLog -Message "GUI Element 'ButtonQuickOUHierarchy' nicht gefunden. Event-Handler nicht zugewiesen." -Type Warning
    }
    ...
    if ($null -ne $Global:ListBoxSelectAttributes) {
        ...
        try {
            ...
            $DefaultAttributes | ForEach-Object {
                ...
                $null = $Global:ListBoxSelectAttributes.Items.Add($listBoxItem)
                if ($_ -in @("DisplayName", "SamAccountName", "Enabled")) { # Standardmäßig ausgewählte Attribute
                    $null = $Global:ListBoxSelectAttributes.SelectedItems.Add($listBoxItem)
                }
            }
        } catch {
    ...
    if ($null -ne $Global:ComboBoxFilterAttribute) {
        try {
            ...
            $FilterableAttributes | ForEach-Object { $null = $Global:ComboBoxFilterAttribute.Items.Add($_) }
    ...
    # Sicherstellen, dass alle Zähler zurückgesetzt werden und UI initialisiert ist
    try {
        if ($null -ne $Global:TotalResultCountText) {
            $Global:TotalResultCountText.Text = "0"
        }
        ...
        Write-Host "UI-Initialisierung abgeschlossen. Zeige Fenster an..." -ForegroundColor Green
        
        # Fenster anzeigen
        $result = $Global:Window.ShowDialog()
        Write-Host "Fenster wurde geschlossen. Rückgabewert: $result" -ForegroundColor Cyan
        Write-Host "Initialize-ADReportForm gibt TRUE zurück (normales Ende)" -ForegroundColor Green
        
        return $true
        
    } catch {
        Write-Error "Fehler beim Finalisieren der UI-Initialisierung: $($_.Exception.Message)"
        return $false
    }
}
...
Function Update-ResultVisualization {
        ...
        if ($ResultsArray.Count -gt 0) {
            ...
            if ($firstObject.PSObject.Properties.Name -contains "ObjectClass") {
                ...
                if ($objectClass -eq "user") {
                    ...
                    $enabledUsers = $ResultsArray | Where-Object {$_.Enabled -eq $true}
                    $disabledUsers = $ResultsArray | Where-Object {$_.Enabled -eq $false}
                    ...
                    if ($firstObject.PSObject.Properties.Name -contains "LockedOut") {
                        $lockedUsers = $ResultsArray | Where-Object {$_.LockedOut -eq $true}
                    ...
                    if ($firstObject.PSObject.Properties.Name -contains "AdminCount") {
                        $adminUsers = $ResultsArray | Where-Object {$_.AdminCount -eq 1}
                ...
                elseif ($objectClass -eq "computer") {
                    ...
                    $enabledComputers = $ResultsArray | Where-Object {$_.Enabled -eq $true}
                    $disabledComputers = $ResultsArray | Where-Object {$_.Enabled -eq $false}
                    ...
                    if ($firstObject.PSObject.Properties.Name -contains "InactiveDays") {
                        $inactiveComputers = $ResultsArray | Where-Object {$_.InactiveDays -gt 30}
                ...
                elseif ($objectClass -eq "group") {
                    ...
                    if ($firstObject.PSObject.Properties.Name -contains "GroupCategory") {
                        $securityGroups = $ResultsArray | Where-Object {$_.GroupCategory -eq "Security"}
                        $distributionGroups = $ResultsArray | Where-Object {$_.GroupCategory -eq "Distribution"}
                    ...
                    $userData = @{"Total" = $ResultsArray.Count}
                }
            }
            else {
    ...
    $null = $Global:ListBoxSelectAttributes.add_SelectionChanged($script:ListBoxSelectionChangedHandler)

    $null = $RadioButtonUser.add_Checked({
        ...
        $UserFilterAttributes | ForEach-Object { $null = $Global:ComboBoxFilterAttribute.Items.Add($_) }
        if ($Global:ComboBoxFilterAttribute.Items.Count -gt 0) { 
            $Global:ComboBoxFilterAttribute.SelectedIndex = 0 
        }
        ...
        $UserAttributes | ForEach-Object {
            ...
            $newItem.Content = $_ 
            $null = $Global:ListBoxSelectAttributes.Items.Add($newItem)
        }
    ...
    $null = $RadioButtonGroup.add_Checked({
        $Global:ListBoxSelectAttributes.IsEnabled = $true
        ...
        $GroupFilterAttributes | ForEach-Object { $null = $Global:ComboBoxFilterAttribute.Items.Add($_) }
        if ($Global:ComboBoxFilterAttribute.Items.Count -gt 0) { 
            $Global:ComboBoxFilterAttribute.SelectedIndex = 0 
        }
    ...
    $null = $RadioButtonComputer.add_Checked({
        $Global:ListBoxSelectAttributes.IsEnabled = $true
        ...
        $ComputerFilterAttributes | ForEach-Object { $null = $Global:ComboBoxFilterAttribute.Items.Add($_) }
        if ($Global:ComboBoxFilterAttribute.Items.Count -gt 0) { 
            $Global:ComboBoxFilterAttribute.SelectedIndex = 0 
        }
    ...
    $null = $RadioButtonGroupMemberships.add_Checked({
    ...
    $null = $ButtonQueryAD.add_Click({
        ...
        try {
            ...
            $null = $ButtonQuickAllUsers.add_Click({
    ...
    $null = $ButtonQuickDisabledUsers.add_Click({
    ...
    $null = $ButtonQuickLockedUsers.add_Click({
    ...
    $null = $ButtonQuickGroups.add_Click({
    ...
    $null = $ButtonQuickSecurityGroups.add_Click({
    ...
    $null = $ButtonQuickDistributionGroups.add_Click({
    ...
    $null = $ButtonQuickNeverExpire.add_Click({
    ...
    $null = $ButtonQuickInactiveUsers.add_Click({
    ...
    $null = $ButtonQuickAdminUsers.add_Click({
    ...
    $null = $ButtonQuickComputers.add_Click({
    ...
    $null = $ButtonQuickInactiveComputers.add_Click({
    ...
    $null = $ButtonQuickWeakPasswordPolicy.add_Click({
    ...
    $null = $ButtonQuickRiskyGroupMemberships.add_Click({
    ...
    $null = $ButtonQuickPrivilegedAccounts.add_Click({
    ...
    $null = $ButtonQuickFSMORoles.add_Click({
    ...
    $null = $ButtonQuickDCStatus.add_Click({
    ...
    $null = $ButtonQuickReplicationStatus.add_Click({
    ...
    $null = $ButtonExportCSV.add_Click({
    ...
    $null = $ButtonExportHTML.add_Click({
...
"@
    ...
    if ($null -ne $Global:ButtonClose) {
        $null = $Global:ButtonClose.add_Click({
            Write-ADReportLog -Message "Beende easyADReport..." -Type Info
            $Global:Window.Close()
            # In PowerShell kann manchmal ein explizites Beenden erforderlich sein
...
Function Start-ADReportGUI {
    ...
    # Zusätzliche Encoding-Konfiguration
    try {
        ...
        Write-Warning "Could not configure encoding: $($_.Exception.Message)"
    }
    
    Write-Host "easyADReport v0.2.4 wird gestartet..." -ForegroundColor Green
    ...
    try {
        ...
        Write-Host "Rufe Initialize-ADReportForm auf..." -ForegroundColor Yellow
        $initResult = Initialize-ADReportForm -XAMLContent $Global:XAML
        
        # Behandle Array-Rückgabewerte korrekt
        $actualResult = $false
        if ($initResult -is [Array]) {
            # Suche nach $true im Array (das letzte Element sollte das eigentliche Ergebnis sein)
            $actualResult = $initResult | Where-Object { $_ -eq $true } | Select-Object -First 1
            if ($actualResult -eq $true) {
                $actualResult = $true
            } else {
                $actualResult = $false
            }
        } else {
            $actualResult = $initResult
        }
        
        Write-Host "Initialize-ADReportForm Ergebnis verarbeitet: '$actualResult'" -ForegroundColor Cyan
        
        # Prüfe auf expliziten Fehler (nur wenn $false zurückgegeben wird)
        if ($actualResult -eq $false) {
            Write-Error "GUI konnte nicht initialisiert werden. Bitte überprüfen Sie das XAML und die WPF-Assemblies."
            Write-Host "Mögliche Lösungen:" -ForegroundColor Yellow
            Write-Host "1. Stellen Sie sicher, dass das ActiveDirectory-Modul verfügbar ist" -ForegroundColor Cyan
            Write-Host "2. Führen Sie das Skript als Administrator aus" -ForegroundColor Cyan
            Write-Host "3. Überprüfen Sie, ob .NET Framework 4.5+ installiert ist" -ForegroundColor Cyan
            Write-Host "Press any key to exit..." -ForegroundColor Red
            $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
            return
        } else {
            # Wenn wir hier ankommen, war die Initialisierung erfolgreich
            # Das Fenster wurde bereits angezeigt und geschlossen - das ist normal
            Write-Host "easyADReport wurde erfolgreich ausgeführt und beendet." -ForegroundColor Green
        }
        
    } catch {
        Write-Error "Unerwarteter Fehler beim Starten der GUI: $($_.Exception.Message)"
        Write-Host "Stack Trace: $($_.ScriptStackTrace)" -ForegroundColor Red
        Write-Host "Press any key to exit..." -ForegroundColor Red
        $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
        return
